...
def check_expected_field_properties(field):
    ...
    if field_id in ['gc', 'length', 'ncount'] or re.match(r'.+_cov', field_id):
        assert field['type'] == 'variable', "expected %s.type to be 'variable'" % field_id
        assert field['range'][0] >= 0, "expected %s.range[0] to be >= 1" % field_id
        if field_id in ['length', 'ncount'] or re.match(r'.+read_cov', field_id):
            assert field['datatype'] == 'integer', "expected %s.datatype to be 'integer'" % field_id
        else:
            assert field['datatype'] == 'float', "expected %s.datatype to be 'float'" % field_id
    elif re.match(r'.+_busco', field_id):
        assert field['type'] == 'multiarray', "expected %s.type to be 'multiarray'" % field_id
        assert field['datatype'] == 'mixed', "expected %s.datatype to be 'mixed'" % field_id
        assert 'Busco id' in field['headers'], "expected to find 'Busco id' in %s.headers" % field_id
        assert 'Status' in field['headers'], "expected to find 'Status' in %s.headers" % field_id
        assert 'category_slot' in field, "expected to find 'category_slot' in %s" % field_id
        message = "expected %s.category_slot to match index of 'Status' in %s.headers" % (field_id, field_id)
        ...
        if re.match(r'.+_cindex', field_id):
            ...
            if re.match(r'.+_.+_positions', field_id):
                assert field['datatype'] == 'string', "expected %s.datatype to be 'string'" % field_id
                assert 'name' in field['headers'], "expected to find 'name' in %s.headers" % field_id
                assert 'category_slot' in field, "expected to find 'category_slot' in %s" % field_id
                message = "expected %s.category_slot to match index of 'name' in %s.headers" % (field_id, field_id)
                assert field['category_slot'] == field['headers'].index('name'), message
                assert 'linked_field' in field, "expected to find 'linked_field' in %s" % field_id
            else:
                assert field['datatype'] == 'mixed', "expected %s.datatype to be 'mixed'" % field_id
                assert 'taxid' in field['headers'], "expected to find 'taxid' in %s.headers" % field_id
                assert 'score' in field['headers'], "expected to find 'score' in %s.headers" % field_id
                assert 'start' in field['headers'], "expected to find 'start' in %s.headers" % field_id
                assert 'end' in field['headers'], "expected to find 'end' in %s.headers" % field_id
                assert 'subject' in field['headers'], "expected to find 'subject' in %s.headers" % field_id
                assert 'index' in field['headers'], "expected to find 'index' in %s.headers" % field_id
...
def generate_data_schema(field, meta, data_schemas, data):
    ...
    if field['type'] == 'variable':
        ...
        if 'category_slot' in field:
            try:
                schema_values['items']['items']['items'] = [{} for x in field['headers']]
                category = {'type': 'integer', 'minimum': 0, 'maximum': len(data['keys']) - 1}
                schema_values['items']['items']['items'][field['category_slot']] = category
            except KeyError as err:
                show_error(err, 5)
